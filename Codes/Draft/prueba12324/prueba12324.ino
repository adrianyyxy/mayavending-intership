...
{
  Serial2.write(0x05);
  //Serial.println("Send 05h...Waiting...");
  Serial.println("05h");
 
  if(Serial2.readBytesUntil(0x30,buffstore,16)>0){
    
    Serial.println("Entro a ciclo, recibio 2 bytes");
    for(int i=0;i<16;i++){
      Serial.println(buffstore[i],HEX);
    }
    ...
    if(buffstore[0] == 0x10 && buffstore[1] == 0x30)
    {
      ...
      if(Serial2.readBytesUntil(0x30,buffstore,2)==2){
  ...
  if(Serial2.read() == 0x05)
  {
    ...
    if(buffstore[0] == 0x10 && buffstore[1] == 0x01)
    {
      ...
      if(buffstore[0] == 0x30 && buffstore[1] == 0x30)
      {
        ...
        store_hex();  //store_hex(10);

        /* --------------------Revision--------------------- */
        //R 0 0
        //52 30 30

        store_hex();  //store_hex(3);
